...
void write_superblock(int fd)
{
	...
	superblock.s_inodes_count = NUM_INODES;					   // changed this
	superblock.s_blocks_count = NUM_BLOCKS;					   // changed this -1
	superblock.s_r_blocks_count = 0;						   // no change
	superblock.s_free_blocks_count = NUM_FREE_BLOCKS;		   // changed this
	superblock.s_free_inodes_count = NUM_FREE_INODES;		   // changed this
	superblock.s_first_data_block = SUPERBLOCK_BLOCKNO;		   // changed this /* First Data Block */
	superblock.s_log_block_size = 0;						   // no change				/* 1024 */
	superblock.s_log_frag_size = 0;							   // no change				/* 1024 */
	superblock.s_blocks_per_group = 8192;					   // NUM_BLOCKS * 8 //changed this
	superblock.s_frags_per_group = 8192;					   // NUM_BLOCKS * 8 //changed this
	superblock.s_inodes_per_group = NUM_INODES;				   // changed this
	superblock.s_mtime = 0;									   /* Mount time */
	superblock.s_wtime = current_time;						   /* Write time */
	superblock.s_mnt_count = 0;								   /* Number of times mounted so far */
	superblock.s_max_mnt_count = -1; /* Make this unlimited */ //-1 means unlimited
	...
	superblock.s_checkinterval = 1;							   // forces checks by making them every one second						   /* Force checks by making them every 1 second */
...
void write_block_group_descriptor_table(int fd)
{
	...
	block_group_descriptor.bg_used_dirs_count = 2; // because we have two directories
...
void write_block_bitmap(int fd)
{
	...
	{
		map_value[i] = 0xFF; // recall every 2 hex chars are one byte
	}

	map_value[2] = 0b01111111; // MSB free
	for (int i = 3; i < 127; i++)
	{
		map_value[i] = 0;
	}

	map_value[127] = 0b10000000; // MSB is 1023rd bit, corresponding to block 1024 which is unavailable
...
void write_inode_table(int fd)
{
	...
	// hello-world file and hello symlink:
	// UID: 1000
	// GID: 1000
	...
	root_inode.i_mode = EXT2_S_IFDIR // Directory permissions
						| EXT2_S_IRUSR | EXT2_S_IWUSR | EXT2_S_IXUSR | EXT2_S_IRGRP | EXT2_S_IXGRP | EXT2_S_IROTH | EXT2_S_IXOTH;
	root_inode.i_uid = 0;
	root_inode.i_size = 1024; // BLOCK_SIZE????
	...
	hello_sym_inode.i_blocks = 0; // No blocks needed for symlink data
	hello_sym_inode.i_links_count = 1;

	char *s = "hello-world";
	size_t len = strlen(s);
	memcpy(&hello_sym_inode.i_block, s, len); // Store symlink target in inode
	write_inode(fd, HELLO_INO, &hello_sym_inode);

	//************************** hello world FILE *******************************
	struct ext2_inode hello_world_file_inode = {0};
	hello_world_file_inode.i_mode = EXT2_S_IFREG // Regular file permissions
									| EXT2_S_IRUSR | EXT2_S_IWUSR | EXT2_S_IRGRP | EXT2_S_IROTH;
	hello_world_file_inode.i_uid = 1000;
	hello_world_file_inode.i_size = 12; // "Hello world\n" is 12 bytes
	...
	hello_world_file_inode.i_blocks = 2; // 2 blocks (512 bytes each)
...
void write_root_dir_block(int fd)
{
	...
	// Move the file descriptor to the start of the root directory block
	...
	// Initialize the remaining bytes in the block
	ssize_t bytes_remaining = BLOCK_SIZE; // 1024 bytes

	// Create and write the root directory entry for "."
	...
	// Create and write the parent directory entry for ".."
	...
	// Create and write the entry for "hello-world" file
	...
	// Create and write the entry for "hello" symlink
	...
	// Create and write the entry for "lost+found" directory
	...
	// Fill the remaining space in the block with zeros
...
void write_hello_world_file_block(int fd)
{
	...
	// // Calculate the byte offset for the block where the hello-world file will be written
	// off_t off = BLOCK_OFFSET(HELLO_WORLD_FILE_BLOCKNO);
	// // Move the file descriptor to the calculated offset
	// off = lseek(fd, off, SEEK_SET);
	// if (off == -1)
	// {
	// 	errno_exit("lseek"); // Exit if lseek fails
	// }

	// // Initialize a buffer to hold the data to be written to the block
	// u8 hello_world_file_buffer[BLOCK_SIZE] = {0};
	// // Set up the string to be written to the file
	// char *s = "Hello world\n";
	// // Calculate the length of the string (should be 12 bytes)
	// size_t len = strlen(s);
	// // Copy the string into the buffer
	// memcpy(hello_world_file_buffer, s, len);

	// // Write the entire buffer to the file
	// if (write(fd, hello_world_file_buffer, BLOCK_SIZE) != BLOCK_SIZE)
	// {
	// 	errno_exit("write"); // Exit if write fails
	// }
